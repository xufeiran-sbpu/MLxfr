...
def get_Phi_2_level(idx_dic,it,threshold,X, y,local):
    ...
    for k in range(number_group):
       ...
       # print("group,coalition:",k,len(coalition))
        ...
        if local!=1:
            if(k==0):
                plotplayerlist=['mean compactivity', 'mean concavity', 'worst compactness', 'worst concave', 'worst concavepoints',
             'worst symmetry']
                plotgraph(plotplayerlist, [a/sum(forplot) for a in forplot], [0,0,0,0,0,0])
